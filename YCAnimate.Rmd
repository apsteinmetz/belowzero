...
```{r}
one_date<- as.Date("2012-01-01")
  one_period <- allYields %>% filter(date > one_date) %>% filter(month(date)==8)
  p <- ggplot(one_period, aes(tenor, country))
  p<- p + geom_tile(aes(fill = yield), colour = "white")
  p<- p + scale_fill_gradientn(colours=c("red","white","steelblue"),
                            values=c(0,zeroPos,1),
                            na.value = "white")
  p<- p + expand_limits(fill=c(yieldRange[1]*1.1,yieldRange[2]))
  p<- p + labs(title="Global Government Yield Curves Go Below Zero",
               x="Years to Maturity",
               y="Countries")
  p<-p+facet_wrap(~date)+theme(axis.text.y = element_blank())
